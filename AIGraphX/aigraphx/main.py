                    ...
                    try:
                        ...
                        if body_text:
                            try:
                                ...
                                logger.debug(
                                    f"→ 请求体 [{request_id}] (原始): {body_text[:1000]}..."
                                )
                    except UnicodeDecodeError:
                        logger.debug(
                            f"→ 请求体 [{request_id}]: <二进制数据，长度 {len(body)} 字节>"
                        )
...
async def global_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    logger.exception(
        f"全局异常处理器捕获到异常 [{request_id}] {request.method} {request.url.path}"
    )
