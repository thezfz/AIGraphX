...
from logging.config import fileConfig # 从 logging.config 导入 fileConfig，用于加载日志配置
from typing import Optional, Union, cast # 导入类型提示
from dotenv import load_dotenv # 导入 python-dotenv 库，用于从 .env 文件加载环境变量

from sqlalchemy import engine_from_config # 从 SQLAlchemy 导入，用于根据配置创建数据库引擎
...
project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..")) # 计算项目根目录路径
...
if DB_URL: # 如果环境变量中设置了 DATABASE_URL
    ...
    try:
        ...
        # 方式二：直接作为模块属性 (config.DATABASE_URL)
            ...
            DB_URL = str(getattr(config, "DATABASE_URL", "")) # 获取属性值，确保是字符串
...
if not DB_URL: # 如果经过环境变量和配置检查后，DB_URL 仍然无效
    ...
    raise ImportError(
        "DATABASE_URL 未在环境变量中设置，也无法从配置中加载。"
    )
