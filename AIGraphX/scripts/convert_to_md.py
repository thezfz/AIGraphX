...
def convert_to_markdown() -> None:
        ...
        try:
            ...
            if source_path.is_file():
                ...
                try:
                    ...
                    logging.info(f"已转换: {relative_path} -> {target_path.relative_to(TARGET_DIR)}")
                    converted_count += 1
                except Exception as write_err:
                    logging.error(f"写入 Markdown 文件 {target_path} 时出错: {write_err}")
                    skipped_count += 1
    ...
    logging.info(f"转换完成。成功转换 {converted_count} 个文件，跳过 {skipped_count} 个文件/目录。")
