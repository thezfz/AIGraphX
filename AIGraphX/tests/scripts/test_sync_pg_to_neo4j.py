...
async def insert_pg_data(repo: PostgresRepository) -> Tuple[int, int]:
    ...
    async with repo.pool.connection() as conn:
        ...
        await conn.commit()
        print(
            f"[DEBUG] insert_pg_data: Inserted papers with IDs {paper1_id}, {paper2_id}"
        )
...
async def test_run_sync_integration(
    ...
    print(
        f"[DEBUG] test_run_sync_integration: PG data inserted, paper IDs: {paper1_id}, {paper2_id}"
    )
    ...
    async with neo4j_repo.driver.session(database=neo4j_repo.db_name) as session:
        ...
        assert record_hf1 is not None, (
            f"Neo4j: Model {TEST_HF_MODEL_1['hf_model_id']} not found"
        )
        ...
        assert record_hf2 is not None, (
            f"Neo4j: Model {TEST_HF_MODEL_2['hf_model_id']} not found"
        )
        ...
        assert record_link["c"] == 1, (
            f"Expected 1 M1->P1 MENTIONS relationship, found {record_link['c']}"
        )
        ...
        # --- 插入额外的 PWC 关系数据到 PG ---
        print(
            "[DEBUG] test_run_sync_integration: Inserting PWC relations into PG for Paper 1..."
        )
        ...
        assert record_link_2 is not None, (
            "Neo4j: Relationship M1->P2 check failed after second sync"
        )
        ...
        assert record_link_3 is not None, (
            "Neo4j: Relationship M2->P2 check failed after second sync"
        )
