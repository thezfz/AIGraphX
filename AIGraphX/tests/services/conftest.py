...
from unittest.mock import (
    AsyncMock,
    MagicMock,
)  # 导入模拟工具，AsyncMock 用于异步，MagicMock 用于同步或通用
...
def mock_pg_repo() -> MagicMock:
    ...
    async def mock_get_details_by_ids(
        paper_ids: List[int],
        scores: Optional[Dict[int, float]] = None,  # 可选的分数映射
    ) -> List[Dict[str, Any]]:
        """模拟根据 ID 列表获取论文详情。"""
        results = []
    ...
    async def mock_search_papers_keyword_return_dicts(
        ...
        if final_date_from:
            filtered_details = [
                ...
                and p.get("published_date") >= final_date_from
            ]
        if final_date_to:
            filtered_details = [
                p
        ...
        if final_area:
            ...
            filtered_details = [
                p for p in filtered_details if p.get("area") == final_area
            ]
