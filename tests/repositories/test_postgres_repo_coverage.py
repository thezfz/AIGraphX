...
async def insert_hf_model(pool: AsyncConnectionPool, model_data: Dict[str, Any]) -> None:
    ...
    if 'hf_last_modified' in model_data and isinstance(model_data['hf_last_modified'], str):
         vals[cols.index('hf_last_modified')] = datetime.fromisoformat(model_data['hf_last_modified'])
    ...
    try:
        ...
        logger.debug(f"Helper inserted/updated HF model: {model_data.get('hf_model_id')}")
    except Exception as e:
        # Log using hf_model_id
        logger.error(f"Error in helper insert_hf_model for {model_data.get('hf_model_id')}: {e}", exc_info=True)
...
async def test_save_hf_models_batch_insert(repository: PostgresRepository, db_pool: AsyncConnectionPool) -> None:
    ...
    details1 = await repository.get_hf_models_by_ids([cast(str, test_hf_model_data_1["hf_model_id"])])
    ...
    # Use hf_model_id
    details2 = await repository.get_hf_models_by_ids([cast(str, test_hf_model_data_2["hf_model_id"])])
    assert len(details2) == 1
    assert details2[0]["hf_pipeline_tag"] == test_hf_model_data_2["hf_pipeline_tag"]

async def test_save_hf_models_batch_update(repository: PostgresRepository, db_pool: AsyncConnectionPool) -> None:
    ...
    details3 = await repository.get_hf_models_by_ids([cast(str, test_hf_model_data_3["hf_model_id"])])
...
async def test_search_models_by_keyword_found(repository: PostgresRepository, db_pool: AsyncConnectionPool) -> None:
    ...
    assert model_ids == {test_hf_model_data_1["hf_model_id"], test_hf_model_data_3["hf_model_id"]}
...
async def test_search_models_by_keyword_pagination(repository: PostgresRepository, db_pool: AsyncConnectionPool) -> None:
    ...
    # Page 1, limit 2
    results_list, total = await repository.search_models_by_keyword("model", limit=2, skip=0)
    assert total == 3
    assert len(results_list) == 2
    # Use hf_model_id
    assert results_list[0]["hf_model_id"] == test_hf_model_data_2["hf_model_id"]
    ...
    # Page 2, limit 2
    results_list, total = await repository.search_models_by_keyword("model", limit=2, skip=2)
    assert total == 3
    assert len(results_list) == 1
    # Use hf_model_id
    assert results_list[0]["hf_model_id"] == test_hf_model_data_3["hf_model_id"]

async def test_search_models_by_keyword_not_found(repository: PostgresRepository, db_pool: AsyncConnectionPool) -> None:
    ...
    results_list, total = await repository.search_models_by_keyword("nonexistentkeyword")
    assert total == 0
    assert len(results_list) == 0


async def test_get_all_hf_models_for_sync(repository: PostgresRepository, db_pool: AsyncConnectionPool) -> None:
...
async def test_get_all_models_for_indexing(repository: PostgresRepository, db_pool: AsyncConnectionPool) -> None:
    ...
    expected_text = {
        cast(str, data1["hf_model_id"]): json.dumps(data1["hf_tags"]) + cast(str, data1["hf_pipeline_tag"]),
        cast(str, data2["hf_model_id"]): json.dumps(data2["hf_tags"]) + cast(str, data2["hf_pipeline_tag"]),
...
async def test_get_tasks_for_papers_not_found(repository: PostgresRepository, db_pool: AsyncConnectionPool) -> None:
...
async def test_get_datasets_for_papers_not_found(repository: PostgresRepository, db_pool: AsyncConnectionPool) -> None:
...
async def test_get_repositories_for_papers_not_found(repository: PostgresRepository, db_pool: AsyncConnectionPool) -> None:
...
async def test_upsert_paper_minimal_data(repository: PostgresRepository, db_pool: AsyncConnectionPool) -> None:
...
async def test_fetch_data_cursor_error_handling(repository: PostgresRepository) -> None:
    ...
    async def mock_fetch_data_cursor_raises(self: Any, query: str, params: Optional[tuple] = None, batch_size: int = 100) -> AsyncGenerator[Dict[str, Any], None]:
    ...
    # 使用patch方法mock
    with patch.object(PostgresRepository, 'fetch_data_cursor', mock_fetch_data_cursor_raises):
        # 测试使用异常生成器的情况
        results = []
...
async def test_get_tasks_for_papers_with_data(repository: PostgresRepository, db_pool: AsyncConnectionPool) -> None:
...
async def test_get_datasets_for_papers_with_data(repository: PostgresRepository, db_pool: AsyncConnectionPool) -> None:
...
async def test_get_repositories_for_papers_with_data(repository: PostgresRepository, db_pool: AsyncConnectionPool) -> None:
    ...
    # 插入存储库，使用正确的列名
    repositories = ["https://github.com/example/repo1", "https://github.com/example/repo2"]
    async with db_pool.connection() as conn:
        async with conn.cursor() as cur:
...
async def test_save_hf_models_batch_with_invalid_data(repository: PostgresRepository, db_pool: AsyncConnectionPool) -> None:
    ...
    # 只确认直接SQL插入的模型
    assert len(valid_models) == 1 or len(valid_models) == 0, f"SQL插入结果: {valid_models}"
...
async def test_search_papers_by_keyword_error_handling(repository: PostgresRepository) -> None:
    """Simulate a database error during search_papers_by_keyword."""
    async def mock_method(
        self: Any, query: str, skip: int = 0, limit: int = 10, 
...
async def test_search_models_by_keyword_error_handling(repository: PostgresRepository) -> None:
...
async def test_get_all_paper_ids_and_text_error_handling(repository: PostgresRepository) -> None:
    """Simulate a database error during get_all_paper_ids_and_text."""
    async def mock_generator(self: Any, batch_size: int = 100) -> AsyncGenerator[Tuple[int, str], None]:
        """模拟抛出异常的异步生成器"""
        raise psycopg.DatabaseError("Simulated DB error")
        yield (0, "")  # 永远不会执行到这里
...
async def test_get_all_models_for_indexing_error_handling(repository: PostgresRepository) -> None:
    ...
    with patch.object(PostgresRepository, 'get_all_models_for_indexing', mock_generator):
        results = []
        with pytest.raises(psycopg.DatabaseError):
...
async def test_get_paper_details_by_id_error_handling(repository: PostgresRepository) -> None:
...
async def test_get_papers_details_by_ids_error_handling(repository: PostgresRepository) -> None:
...
async def test_get_all_papers_for_sync_error_simulation(repository: PostgresRepository) -> None:
    ...
    with patch.object(PostgresRepository, 'get_all_papers_for_sync', mock_get_all_papers_for_sync):
