...
import numpy as np
from typing import List, Tuple, Dict, Optional, Set, Literal, Union, cast, Any, Sequence, Callable
import json
...
# --- Existing Tests ---
# ... existing code ...
async def test_convert_distance_to_score_negative_distance(search_service: SearchService) -> None:
...
async def test_get_model_details_for_ids_last_modified_invalid_string(
    ...
    model_data = {
        ...
        "pipeline_tag": "t", "author":"a", "library_name":"l", "likes": 0, "downloads":0,
    }
    mock_pg_repo.get_hf_models_by_ids.return_value = [model_data]
...
async def test_get_model_details_for_ids_item_creation_error(
    ...
    valid_data = {
        ...
        "last_modified": datetime(2023, 1, 1),
        "likes": 10,
        "pipeline_tag": "t", "author":"a", "library_name":"l", "downloads":0, # Add required
    }
    invalid_data = {
    ...
    result = await search_service._get_model_details_for_ids(["model-valid", "model-invalid-field"])
...
async def test_filter_results_by_date_empty_input(search_service: SearchService) -> None:
    """测试 _filter_results_by_date 输入为空列表"""
    # 对应行 334
    assert search_service._filter_results_by_date([], None, None) == []

async def test_filter_results_by_date_item_missing_date_attr(search_service: SearchService) -> None:
    ...
    assert search_service._filter_results_by_date(items_with_dict, date(2023,1,1), None) == []

async def test_filter_results_by_date_item_date_is_none(search_service: SearchService) -> None:
    """测试 _filter_results_by_date item 的 published_date 为 None"""
    # 对应行 346
    item_with_none_date = create_dummy_paper(1, None, None, "t", pwc_id="pwc-1") # Use helper
    assert search_service._filter_results_by_date([item_with_none_date], date(2023,1,1), None) == []
...
async def test_apply_sorting_none_sort_key(search_service: SearchService) -> None:
    ...
    paginated, total, _, _ = search_service._apply_sorting_and_pagination(items, None, "desc", 1, 10)
    assert total == 2
    assert paginated == items # Order should be preserved


@pytest.mark.parametrize("sort_by", ["score", "published_date", "title"])
async def test_apply_sorting_papers_none_values(search_service: SearchService, sort_by: PaperSortByLiteral) -> None:
    ...
    paginated, total, _, _ = search_service._apply_sorting_and_pagination(items, sort_by, "desc", 1, 10)
    assert total == 2
...
async def test_apply_sorting_models_none_values(search_service: SearchService, sort_by: ModelSortByLiteral) -> None:
    """测试 _apply_sorting_and_pagination 对模型排序时处理 None 值"""
    item_with_none = create_dummy_model("m1", None, None, None, downloads=None) # Explicit None download
...
async def test_apply_sorting_unsupported_sort_key(search_service: SearchService) -> None:
...
async def test_apply_sorting_unsupported_item_type(search_service: SearchService) -> None:
    """测试 _apply_sorting_and_pagination 使用不支持的 item 类型 (dict)"""
    # 对应行 410
    items: Sequence[Any] = [{"id": 1, "score": 0.5}] # Use dict and Any type hint
    paginated, total, _, _ = search_service._apply_sorting_and_pagination(items, "score", "desc", 1, 10)
    assert total == 0
    assert paginated == []
...
async def test_apply_sorting_sort_key_error(search_service: SearchService) -> None:
    ...
    type(paper_mock).title = property(fget=MagicMock(side_effect=AttributeError("Simulated access error")))

    items = [paper_mock]

    # No need to patch get_sort_key now, the error happens accessing the item's attribute
    paginated, total, _, _ = search_service._apply_sorting_and_pagination(items, "title", "desc", 1, 10)
...
async def test_apply_sorting_type_error(search_service: SearchService) -> None:
    ...
    paginated_ids = [
        item.model_id if isinstance(item, HFSearchResultItem) else getattr(item, 'paper_id', None)
...
async def test_apply_sorting_generic_error(search_service: SearchService) -> None:
    ...
    def mock_sorted(items_list: list, *, key: Optional[Callable]=None, reverse: bool=False) -> list:
    ...
    with patch('builtins.sorted', side_effect=mock_sorted):
        # 调用函数
        paginated, total, _, _ = search_service._apply_sorting_and_pagination(
            items, "published_date", "desc", 1, 10  # 使用有效的排序键
        )
    ...
    paper_ids = {item.paper_id for item in paginated if isinstance(item, SearchResultItem)}
...
async def test_semantic_search_invalid_target(search_service: SearchService, target: str) -> None:
    """测试 perform_semantic_search 使用无效 target"""
    # 对应行 466-482
    result = await search_service.perform_semantic_search("query", cast(SearchTarget, target))
...
async def test_semantic_search_empty_query(search_service: SearchService) -> None:
    ...
    result_models = await search_service.perform_semantic_search("   ", "models") # Query with spaces
    assert isinstance(result_models, PaginatedHFModelSearchResult)
    assert result_models.items == []
    assert result_models.total == 0


async def test_semantic_search_embed_error(search_service: SearchService, mock_embedder: MagicMock) -> None:
    ...
    assert isinstance(result, PaginatedSemanticSearchResult) # Generic empty on embed error
    assert result.items == []
    assert result.total == 0


async def test_semantic_search_embed_returns_none(search_service: SearchService, mock_embedder: MagicMock) -> None:
...
async def test_semantic_search_faiss_not_ready(search_service: SearchService, mock_faiss_repo_papers: MagicMock) -> None:
...
async def test_semantic_search_faiss_id_type_mismatch(search_service: SearchService, mock_faiss_repo_papers: MagicMock) -> None:
...
async def test_semantic_search_faiss_search_error(search_service: SearchService, mock_faiss_repo_papers: MagicMock) -> None:
...
async def test_semantic_search_faiss_no_results(search_service: SearchService, mock_faiss_repo_papers: MagicMock) -> None:
...
async def test_semantic_search_faiss_result_id_type_mismatch(search_service: SearchService, mock_faiss_repo_papers: MagicMock) -> None:
    """测试 perform_semantic_search Faiss 结果中的 ID 类型不匹配"""
    mock_faiss_repo_papers.is_ready.return_value = True
    mock_faiss_repo_papers.id_type = "int"
    mock_faiss_repo_papers.search_similar.return_value = [("id_str", 0.5), (2, 0.4)]

    mock_details_fetch = AsyncMock(return_value=[
        create_dummy_paper(2, 0.8, date(2023,1,2), "B")
    ])
...
async def test_semantic_search_faiss_all_ids_filtered(search_service: SearchService, mock_faiss_repo_papers: MagicMock) -> None:
...
async def test_semantic_search_fetch_details_error(search_service: SearchService, mock_faiss_repo_papers: MagicMock) -> None:
...
async def test_semantic_search_invalid_sort_key(search_service: SearchService, mock_faiss_repo_papers: MagicMock, mock_pg_repo: MagicMock) -> None:
    """测试 perform_semantic_search 使用无效 sort_by"""
    mock_faiss_repo_papers.is_ready.return_value = True
    mock_faiss_repo_papers.id_type = "int"
    mock_faiss_repo_papers.search_similar.return_value = [(1, 0.5), (2, 0.4)] # ID 1 score > ID 2 score
    ...
    # Use patch.object
    with patch.object(search_service, '_get_paper_details_for_ids', mock_details_fetch):
        result = await search_service.perform_semantic_search("query", "papers", sort_by=cast(PaperSortByLiteral, "likes"))
...
@pytest.mark.parametrize("target", ["all", "invalid_target"])
async def test_keyword_search_invalid_target(search_service: SearchService, target: str) -> None:
    """测试 perform_keyword_search 使用无效 target"""
    # 对应行 652
    result = await search_service.perform_keyword_search("query", cast(SearchTarget, target))
...
async def test_keyword_search_papers_missing_pg_method(search_service: SearchService, mock_pg_repo: MagicMock) -> None:
...
async def test_keyword_search_models_missing_pg_method(search_service: SearchService, mock_pg_repo: MagicMock) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("papers", "downloads", "published_date"), # downloads invalid for paper keyword
...
async def test_keyword_search_invalid_sort_key(
    search_service: SearchService, mock_pg_repo: MagicMock, target: SearchTarget, invalid_sort_key: str, default_sort_key: str
) -> None:
    ...
    await search_service.perform_keyword_search("query", target, sort_by=cast(Union[PaperSortByLiteral, ModelSortByLiteral], invalid_sort_key))
...
async def test_keyword_search_pg_search_error(search_service: SearchService, mock_pg_repo: MagicMock) -> None:
...
async def test_keyword_search_papers_item_conversion_error_outer(search_service: SearchService, mock_pg_repo: MagicMock) -> None:
    """测试 perform_keyword_search papers 转换结果时外部捕获异常"""
    # 对应行 749-750
    mock_pg_repo.search_papers_by_keyword.return_value = ("not a list", 1) # Invalid return type
...
async def test_keyword_search_papers_item_conversion_error_inner(search_service: SearchService, mock_pg_repo: MagicMock) -> None:
    """测试 perform_keyword_search papers 单个 item 转换失败"""
    # 对应行 771-774
    valid_paper_data = {"paper_id": 1, "title": "Valid", "summary": "s", "pwc_id": "p1", "published_date": date(2023,1,1), "authors": ["A"], "area": "cs.AI", "pdf_url": None, "score": None} # Ensure valid data
    invalid_paper_data = {"paper_id": 2, "title": "Invalid", "published_date": "bad-date"} # Invalid date format
    mock_pg_repo.search_papers_by_keyword.return_value = ([valid_paper_data, invalid_paper_data], 2)
...
async def test_keyword_search_models_item_conversion_error_inner(search_service: SearchService, mock_pg_repo: MagicMock) -> None:
    """测试 perform_keyword_search models 单个 item 转换失败"""
    # 对应行 798-803 (inside loop)
    valid_model_data = {"model_id": "m1", "likes": 10, "pipeline_tag":"t", "author":"a","library_name":"l", "downloads": 0, "last_modified": datetime(2023,1,1), "score": 0.0, "tags": []} # Ensure valid data
    invalid_model_data = {"model_id": "m2", "likes": "not-an-int"} # Invalid likes type
    mock_pg_repo.search_models_by_keyword.return_value = ([valid_model_data, invalid_model_data], 2)
...
async def test_hybrid_search_no_embedder(
    ...
    kw_result = {"paper_id": 1, "title": "Keyword Only", "summary": "s", "pwc_id":"p1", "published_date": date(2023,1,1), "authors": ["Author"]}
...
async def test_hybrid_search_embed_error(
    ...
    kw_result = {"paper_id": 1, "title": "Keyword Only", "summary": "s", "pwc_id":"p1", "published_date": date(2023,1,1), "authors":["A"], "area": "cs.AI"} # Ensure valid data
    mock_pg_repo.search_papers_by_keyword.return_value = ([kw_result], 1)
    # Mock details fetch (will be called for keyword result ID)
    mock_pg_repo.get_papers_details_by_ids.return_value = [kw_result] # Return the same full data
...
async def test_hybrid_search_keyword_search_error(
    search_service: SearchService, mock_embedder: MagicMock, mock_pg_repo: MagicMock, mock_faiss_repo_papers: MagicMock
) -> None:
    ...
    mock_pg_repo.search_papers_by_keyword.side_effect = Exception("Keyword search failed")
    # Mock details fetch (will be called for semantic result)
    sem_details = {"paper_id": 1, "title": "Semantic Only", "summary": "s", "pwc_id":"p1", "published_date": date(2023,1,1)}
...
async def test_hybrid_search_fetch_details_error(
    ...
    # 对应行 880-886
    mock_embedder.embed.return_value = np.array([0.1, 0.2])
    mock_faiss_repo_papers.search_similar.return_value = [(1, 0.5)] # Semantic finds ID 1
    mock_pg_repo.search_papers_by_keyword.return_value = ([], 0) # Keyword finds nothing
    # Mock details fetch to fail
    mock_pg_repo.get_papers_details_by_ids.side_effect = Exception("Fetch details failed")
...
async def test_hybrid_search_rrf_logic_semantic_only(
    ...
    mock_embedder.embed.return_value = np.array([0.1, 0.2])
    mock_faiss_repo_papers.search_similar.return_value = [(1, 0.5), (2, 0.4)] # Lower distance = better score
...
async def test_hybrid_search_rrf_logic_keyword_only(
    ...
    kw_result1 = {"paper_id": 1, "title": "T1", "summary":"s1", "pwc_id":"p1", "authors": ["A"], "published_date": date(2023,1,1)}
    kw_result2 = {"paper_id": 2, "title": "T2", "summary":"s2", "pwc_id":"p2", "authors": ["B"], "published_date": date(2023,1,2)}
...
async def test_hybrid_search_item_creation_error(
    ...
    if hasattr(mock_pg_repo, 'paper_details_map') and 1 in mock_pg_repo.paper_details_map:
    ...
    try:
        ...
        assert result.total == 0 # Total should reflect items successfully created and filtered
    finally:
...
async def test_hybrid_search_sorting_default(
    ...
    kw_result2 = {"paper_id": 2, "title": "T2", "summary":"s2", "pwc_id":"p2", "authors": ["B"], "published_date": date(2023,1,2)}
    kw_result3 = {"paper_id": 3, "title": "T3", "summary":"s3", "pwc_id":"p3", "authors": ["C"], "published_date": date(2023,1,3)}
    # 重置side_effect以避免默认实现
    mock_pg_repo.search_papers_by_keyword.side_effect = None
    mock_pg_repo.search_papers_by_keyword.return_value = ([kw_result2, kw_result3], 2)
    # Details
    details1 = {"paper_id": 1, "title": "T1", "summary":"s1", "pwc_id":"p1", "authors": ["A"], "published_date": date(2023,1,1)}
    details2 = {"paper_id": 2, "title": "T2", "summary":"s2", "pwc_id":"p2", "authors": ["B"], "published_date": date(2023,1,2)}
    details3 = {"paper_id": 3, "title": "T3", "summary":"s3", "pwc_id":"p3", "authors": ["C"], "published_date": date(2023,1,3)}
    mock_pg_repo.get_papers_details_by_ids.return_value = [details1, details2, details3]

    result = await search_service.perform_hybrid_search("query", filters=None) # No filter -> default sort
...
async def test_hybrid_search_sorting_with_filter(
    ...
    mock_embedder.embed.return_value = np.array([0.1, 0.2])
    mock_faiss_repo_papers.search_similar.return_value = [(1, 0.5)]
    details1 = {"paper_id": 1, "title": "ABC", "summary":"s1", "pwc_id":"p1", "published_date": date(2023, 1, 5), "authors": ["A"], "area": "CV"}
    details2 = {"paper_id": 2, "title": "XYZ", "summary":"s2", "pwc_id":"p2", "published_date": date(2023, 1, 1), "authors": ["B"], "area": "NLP"}
    # 重置side_effect以避免默认实现
    mock_pg_repo.search_papers_by_keyword.side_effect = None
